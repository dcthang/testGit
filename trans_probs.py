...
custom_config = r'-l vie '  # --psm 1,3,4, # --oem 3 # vie+eng+equ # --psm 1 --oem 1 good at --psm 11
...
#        if not is_intersect:
    ...
    for i, box in enumerate(pred_boxes):
        ...
        if hmer is not None:
            ...
            #_rs, scores, n_best, times = onmt.run([{"id": 1, "src": image}])
            latex = _rs[0]
        # show debugging
...
#        print("latex, ", latex)
    ...
    #
    # MetadataCatalog.get("mfr_train").set(thing_classes=["formula"])
    # metadata_val = MetadataCatalog.get("mfr_train") # for d in random.sample(dataset_dicts, 3):
    #
    # v = Visualizer(im[:, :, ::-1],
...
def detect_fomular_x(im, model='onmt'):
    """
...
def predict_final(filename):
    ...
    print(f"formulas = {formulas} \r \n")
    lines, lines_bbox = detect_text(im)

    print(f"lines = {lines} , lines_bbox = {lines_bbox}")
    ...
    for i in range(len(lines)):
        ...
        results.append(res)
    #print("Cong thuc do dc: {0} ".format(detect_fomular_x(im)))
    return "\n".join(results)
    # cv2.waitKey()
