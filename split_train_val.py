...
def list_files(directory):
    ...
    """
    return [
        (f, path.join(path.splitext(f)[0] + ".xml"))
        for f in pathlib.Path(directory).iterdir()
        if f.is_file() and not f.name.startswith(".") and f.name.endswith((".jpg", ".png"))
    ]
...
if __name__ == '__main__':
    ratio("Toan6_tap1", output="output", seed=1337, ratio=(.9, .1), group_prefix=None)
